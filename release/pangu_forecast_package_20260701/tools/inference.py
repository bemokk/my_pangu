...
def preload_cuda_dependencies(ort):
    """Load Windows CUDA/cuDNN DLLs before ONNX Runtime creates a session."""
    try:
        import torch
    ...
    if torch is None and os.name == "nt":
        print(
            "Warning: install a CUDA-enabled PyTorch build so ONNX Runtime "
            "can find the CUDA and cuDNN DLLs."
        )
...
def infer(start_time, forecast_hour, dataType):
    ...
    if not os.path.exists(input_dir):
        raise FileNotFoundError(f"Input dir doesn't exist: {input_dir}")
    ...
    if missing_models:
        ...
        raise FileNotFoundError(f"Pangu model files are missing:\n{missing}")
    ...
    # 3. 解码部分 .nc
    print("====== 正在解码 .nc 文件 ======")
    from .decode import surface, upper
